...
export default function App() {
  ...
  const handleDownload = async (format = 'pdf') => {
    ...
    // Choose content based on active tab
    ...
    const fileName = `${activeTab.name.replace(/\s+/g, '_')}_SkillSyncAI`;
    
    try {
      if (format === 'pdf') {
        const html2pdf = (await import('html2pdf.js')).default;
        
        // Create a hidden "printable" container for better formatting
        ...
        // We use formatMarkdown to make the PDF look like the UI preview but optimized for print
        element.innerHTML = `
          <div style="margin-bottom: 20px; border-bottom: 2px solid #333; padding-bottom: 10px;">
            <h1 style="margin: 0; color: #333; font-size: 24pt;">${activeTab.name}</h1>
            <p style="margin: 5px 0 0 0; color: #666; font-size: 10pt;">Generated by SkillSync AI</p>
          </div>
          <div style="font-size: 11pt; color: #111;">
            ${formatMarkdown(content)}
          </div>
          <div style="margin-top: 50px; border-top: 1px solid #eee; padding-top: 10px; text-align: center; color: #999; font-size: 8pt;">
            Professional analysis provided by SkillSync AI. Visit skillsync-ai.com
          </div>
        `;
      ...
      setError("Failed to generate download. Please check your browser's PDF capabilities.");
    } finally {
      // Small artificial delay for a better UX feeling
                                   ...
                                   <div>
                                    ...
                                    <div className="flex gap-2">
                                     <Button 
                                       ...
                                       title="Copy Text" 
                                       onClick={() => {
                                         navigator.clipboard.writeText(result?.tailored_resume || result?.recommendation || '');
                                   ...
                                   <div>
                                    ...
                                    <div className="flex gap-2">
                                     <Button 
                                       ...
                                       className="hover:bg-muted text-foreground transition-colors" 
                                       title="Copy Text" 
                                       onClick={() => navigator.clipboard.writeText(result?.cover_letter || result?.recommendation || '')}
                                     >
                                       <Copy className="w-4 h-4" />
                                     </Button>
                                     <Button 
